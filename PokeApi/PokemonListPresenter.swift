...
class PokemonListPresenter: PokemonListPresenterProtocol {
    ...
    func fetchPokemonWithQuery(){
        if !pokemonQuery.isEmpty {
            pokemonApi.getPokemonBy(name: pokemonQuery) { [weak self]
                pokemonInfo in
                guard let `self` = self,
    ...
    private func transformToPokemonData(using pokemonInfo: PokemonDetailInfo) -> PokemonData {
        return PokemonData(next: nil, previous: nil, results: [PokemonData.PokemonResult(name: pokemonInfo.name, url: "\(PokeApiProvider.baseURL)\(pokemonInfo.id)")])
    }
    ...
    private func enableButtonForType(using pokemonData: PokemonData) -> ButtonVisibility {
        ...
        return ButtonVisibility(previous: previousResults, next: nextResults) //Default value enables next results only
    }
